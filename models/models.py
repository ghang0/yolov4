...
class SPPLayer(nn.Module):
    ...
    def forward(self, x):
        t1 = torch.abs(x)
        t2 = torch.abs(self.p1(x))
        t3 = torch.abs(self.p2(x))
        y1, _ = torch.max(t1, dim=1, keepdim=True)
        y2, _ = torch.max(t2, dim=1, keepdim=True)
        y3, _ = torch.max(t3, dim=1, keepdim=True)
        return y1+y2+y3
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if mtype == 'convolutional':
                ...
                if 'spp' in mdef:
                    y = SPPLayer()(x)
                    spp.append(y)
            elif mtype in ['upsample', 'maxpool','BatchNorm2d', 'avgpool']:
                x = module(x)

            elif mtype in ['route', 'shortcut', 'scale_channels', 'sam', 'route_lhalf','route2','route3']:
