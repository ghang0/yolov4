...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    balance = [4.0, 1.0, 0.3] if np == 3 else [4.0, 1.0, 0.3, 0.1]  # P3-5 or P3-6
...
def non_max_suppression(prediction, conf_thres=0.001, nms_thres=0.5, merge=False, classes=None, agnostic=False):
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=1280, max_subplots=16):
    ...
    if fname is not None:
        mosaic = cv2.resize(mosaic, (int(ns * w * 0.8), int(ns * h * 0.8)), interpolation=cv2.INTER_AREA)
...
def plot_results(cls=1, start=0, stop=0, labels=(), save_dir=''):  # from utils.utils import *; plot_results()
    if cls>1:
        fig, ax = plt.subplots(2, 5, figsize=(14, 6), tight_layout=True)
        ax = ax.ravel()
        s = ['IoU', 'Obj', 'Class', 'Precision', 'Recall',
             'val IoU', 'val Obj', 'val Class', 'mAP@0.5', 'mAP@0.75']
        ...
        ax = ax.ravel()
        s = ['IoU', 'Obj',  'Precision', 'Recall', 'val IoU', 'val Obj',  'mAP@0.5', 'mAP@0.75']
        files = glob.glob(str(Path(save_dir) / 'results*.txt')) + glob.glob('../../Downloads/results*.txt')
        for fi, f in enumerate(files):
            try:
...
def distillation_loss2(model, targets, output_s, output_t):
    ...
    device = targets.device
    lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
    tcls, tbox, indices, anchors = build_targets(output_s, targets, model)  # targets
    ...
    for i, (ps, pt) in enumerate(zip(output_s, output_t)):  # layer index, layer predictions
        ...
        if nb:  # number of targets
            ...
            psxy = pss[:, :2].sigmoid() * 2. - 0.5
            pswh = (pss[:, 2:4].sigmoid() * 2) ** 2 * anchors[i]
            psbox = torch.cat((psxy, pswh), 1).to(device)  # predicted box

            ptxy = pts[:, :2].sigmoid() * 2. - 0.5
            ptwh = (pts[:, 2:4].sigmoid() * 2) ** 2 * anchors[i]
            ptbox = torch.cat((ptxy, ptwh), 1).to(device)  # predicted box
...
def distillation_loss3(model, targets, output_s, output_t, spp1, spp2):
    reg_m = 0.0
    T = 3.0
    Lambda_cls, Lambda_box, Lambda_pam = 0.0001, 0.001, 0.001
    criterion_st = torch.nn.KLDivLoss(reduction='sum')
    device = targets.device
    lcls, lbox, lobj, lpam = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
    tcls, tbox, indices, anchors = build_targets(output_s, targets, model)  # targets
    reg_ratio, reg_num, reg_nb= 0, 0, 0
    for i, (ps, pt) in enumerate(zip(output_s, output_t)):  # layer index, layer predictions
        b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx
    ...
    for j, (s1, s2) in enumerate(zip(spp1, spp2)):  # layer index, layer predictions
        # print(s1.shape)
        l2_dist = (s1 - s2).pow(2).sum(1)
        # print(s1.shape[-1])
        lpam += l2_dist.sum() / (s1.shape[-1] * s1.shape[-1])
        # print(lpam)

    if reg_nb:
        reg_ratio = reg_num / reg_nb

    return (lcls * Lambda_cls + lbox * Lambda_box) + lpam * Lambda_pam, reg_ratio
